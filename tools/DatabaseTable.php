...
namespace tools;
    ...
    class DatabaseTable {
        ...
        public function orderedFind($field, $value, $order) {
            $stmt = $this->pdo->prepare('SELECT * FROM ' . $this->table . ' WHERE ' . $field . ' = :value' . ' ORDER BY ' . $order . ' ASC;');
            $stmt->setFetchMode(\PDO::FETCH_CLASS, $this->entityClass, $this->entityConstructor);
            $criteria = [
                'value' => $value
            ];
